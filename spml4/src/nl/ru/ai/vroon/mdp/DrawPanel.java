...
public class DrawPanel extends JPanel {
	 ...
	 * @param screenHeight
	 */
	public DrawPanel(MarkovDecisionProblem mdp, int screenWidth,
			int screenHeight) {
		this.mdp = mdp;
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < mdp.getWidth(); i++) {
			for (int j = 0; j < mdp.getHeight(); j++) {
				...
				g2.fillRect(stepSizeX * i, screenHeight - stepSizeY * (j + 1),
						stepSizeX, stepSizeY);

				if (mdp.getStateXPosition() == i
						&& mdp.getStateYPosition() == j) {
					g2.setPaint(Color.BLUE);
					g2.fillOval(stepSizeX * i + stepSizeX / 4, screenHeight
							- stepSizeY * (j + 1) + stepSizeY / 4,
							stepSizeX / 2, stepSizeY / 2);
				}

				g2.setPaint(Color.BLACK);
				g2.drawRect(stepSizeX * i, screenHeight - stepSizeY * (j + 1),
						stepSizeX, stepSizeY);
				if (mdp.isPolicyAdded()) {
					Action a = mdp.getPolicyAction(i, j);
					g2.setPaint(Color.BLACK);
					g2.drawString(a.name(), stepSizeX * i, screenHeight
							- stepSizeY * (j ));
				}
			}
		}
		g2.drawString("Reward: \t\t" + mdp.getReward(), 30, screenHeight + 25);
		g2.drawString("#Actions: \t\t" + mdp.getActionsCounter(), 30,
				screenHeight + 40);
	}
